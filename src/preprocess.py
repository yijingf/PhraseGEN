...
def MIDI_tokenization(midi_dir, df, t_bounds, token_path, max_t_phrase=20, min_n_note=10):
    ...
    for i, row in df.iterrows():
        ...
        for t_st, t_ed in zip(*[[0] + t_bound, t_bound + [t_duration]]):
            ...
            # Skip long segments
            if t_ed - t_st > max_t_phrase:
                continue

            # Trim midi
            pm_seg = trim_midi(pm, t_st, t_ed, by='onset', meta=False)

            # Skip segments with too few notes
...
def main(midi_dir, df, t_bounds, token_path, max_len=600, test_size=0.2):
